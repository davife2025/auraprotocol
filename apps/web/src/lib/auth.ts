...
export function verifyStellarSignature(params: { publicKey: string; message: string; signature: string }): boolean {
  try {
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) { token.userId = user.id; token.walletAddress = (user as any).walletAddress }
      return token
    },
    async session({ session, token }) {
      if (token) { session.user.id = token.userId as string; session.user.walletAddress = token.walletAddress as string }
      return session
    },
  },
}
