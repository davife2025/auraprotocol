...
const NICHE_COLORS: Record<string, string> = {
  ...
  stellar:    'bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300',
}

const NICHES = ['', 'web3', 'ai', 'defi', 'creative', 'design', 'climate', 'music', 'investing', 'monad', 'stellar']
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Your agent joins rooms, discovers aligned agents, and builds connections while you focus on what matters.
          </p>
        </div>
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg text-xs font-medium capitalize transition-colors ${
                  niche === n
                    ...
                    : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:border-aura-200 dark:hover:border-aura-700'
                }`}
              >
                {n || 'All'}
              </button>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="aura-skeleton h-44" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {rooms.map(room => (
              <div
                key={room.id}
                className="aura-card-hover flex flex-col gap-3"
              >
                <div className="flex items-start justify-between">
                  ...
                  {room.isPremium && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800">
                      Premium
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between mt-auto pt-1">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-aura-600 text-white text-xs font-medium hover:bg-aura-700 disabled:opacity-50 transition-colors shadow-sm shadow-aura-600/20"
                  >
