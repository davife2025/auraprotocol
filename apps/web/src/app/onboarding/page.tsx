        ...
        <div className="flex items-center gap-2 mb-8 overflow-x-auto pb-1">
          {STEPS.map((s, i) => (
            <div key={i} className="flex items-center gap-2 shrink-0">
              ...
              {i < STEPS.length - 1 && <div className={`h-0.5 w-6 transition-colors ${i < step ? 'bg-teal-400' : 'bg-gray-200 dark:bg-gray-700'}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-8">
          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              <p className="text-gray-500 text-sm">Your Stellar wallet is your agent's onchain identity anchor.</p>

              {!isInstalled && (
                <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                  <p className="text-sm text-amber-700 dark:text-amber-300">
                    Freighter not detected.{' '}
                    <a href="https://www.freighter.app/" target="_blank" rel="noreferrer" className="underline">Install it here →</a>
                  </p>
                </div>
              )}
              ...
              {isConnected && publicKey ? (
                <div className="flex items-center gap-3 p-3 bg-teal-50 dark:bg-teal-900/20 rounded-xl border border-teal-200 dark:border-teal-800">
                  ...
                  <span className="text-sm text-teal-700 dark:text-teal-300 font-mono">{truncateStellarAddress(publicKey, 8)}</span>
                </div>
              ) : (
                <button onClick={connect} disabled={connecting || !isInstalled}
                  className="w-full py-3 rounded-xl border border-gray-200 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-40 transition-colors">
                  {connecting ? 'Connecting...' : 'Connect Freighter'}
                </button>
              )}

              <button onClick={next} disabled={!isConnected}
                className="w-full py-3 rounded-xl bg-aura-600 text-white font-medium disabled:opacity-40 hover:bg-aura-800 transition-colors">
                Continue
              </button>
            </div>
          )}
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <p className="text-gray-500 text-sm">This is how your agent introduces itself in rooms and meetings.</p>
              <input
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['formal', 'professional', 'casual'].map(style => (
                    <button key={style} onClick={() => setForm(f => ({ ...f, communicationStyle: style }))}
                      className={`py-2 rounded-lg text-sm font-medium capitalize transition-colors ${form.communicationStyle === style ? 'bg-aura-100 dark:bg-aura-900 text-aura-800 dark:text-aura-200 border border-aura-300 dark:border-aura-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'}`}>
                      {style}
                    </button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['conservative', 'moderate', 'aggressive'].map(risk => (
                    <button key={risk} onClick={() => setForm(f => ({ ...f, riskTolerance: risk }))}
                      className={`py-2 rounded-lg text-sm font-medium capitalize transition-colors ${form.riskTolerance === risk ? 'bg-aura-100 dark:bg-aura-900 text-aura-800 dark:text-aura-200 border border-aura-300 dark:border-aura-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'}`}>
                      {risk}
                    </button>
                  ))}
                </div>
            ...
            <div className="space-y-5 text-center">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={createAgent} disabled={loading}
                  className="flex-1 py-3 rounded-xl bg-aura-600 text-white font-medium disabled:opacity-40 hover:bg-aura-800 flex items-center justify-center gap-2">
